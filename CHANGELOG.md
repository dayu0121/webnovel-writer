...
- 新增 `short-story-v1` 原生短故事仓、故事级状态推导、整篇草稿/审读/修订/定稿链和 hash 绑定的交付批准。
- 新增 12 个 `story_*` 主 Agent 工具、短故事总控/审读技能，以及无伪卷章的 Markdown/TXT 计算与冲突安全导出。
- 短故事进入书房书架、搜索和作品视图；长篇目录、卷章视图与既有书仓保持兼容。
- 短故事模式固定为番茄平台：新增带来源、版本、hash 和活动边界的 `Fanqie-short-story@1` 规则包；非番茄规则包/平台会被拒绝。

### Fixed

- 长篇待定稿包在当前审读未完成或证据过期时拒绝组装。
- 长篇定稿批准绑定待定稿包 hash；批准期间包内容变化会拒绝入档。
- 非空时间线/账本/世界书事实在 Git 失败后重跑不再重复追加。
