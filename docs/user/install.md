...
Node、pnpm、DSH 预期版本分别属于上述范围、11.27.1、0.1.5-rc.2；Git 与 PowerShell 7 应能正常输出版本。安装依赖需要网络。Git 用于建书和保存确认后的版本历史；预构建包无需 TypeScript 或 npm 账号。若另一个 DSH 已在使用，请先阅读备份说明，并用不同的 profile 名称安装本工作台。
...
本版精确版本为 `0.1.0-preview.6`；需要固定版本时用 `@0.1.0-preview.6` 替换 `@preview`。公开安装无需 npm 账号。首次创建必须使用 `web` 模板；已有同名 profile 时请换名或先确认用途。
...
打开 [本版 Release](https://github.com/lingfengQAQ/webnovel-writer/releases/tag/scriptor-v0.1.0-preview.6)，下载主包 `linfengqaqtat-dsh-scriptor-0.1.0-preview.6.tgz` 和 `SHA256SUMS`。
...
```powershell
Get-FileHash -Algorithm SHA256 C:/scriptor-dist/linfengqaqtat-dsh-scriptor-0.1.0-preview.6.tgz
```
...
```powershell
dsh --profile scriptor --from-default-profile web --dump-config
dsh plugin --profile scriptor add C:/scriptor-dist/linfengqaqtat-dsh-scriptor-0.1.0-preview.6.tgz
dsh --profile scriptor --dump-config
```
